...
const handleButtonClick = () => {
  ...
  if (zipElement.value.length === 0) {
    document.querySelector('#zip').style.backgroundColor =
      'rgba(255, 0, 0, 0.45)';
...
const getWeather = async (zipCode, countryCode, apiKey) => {
  const response = await fetch(
    `${BASE_URLS.weather}weather?zip=${zipCode},${countryCode}&appid=${apiKey}`
  );
...
const postData = async (data = {}) => {
  try {
    const response = await fetch(`${BASE_URLS.server}/add_weather`, {
      method: 'POST',
...
const getData = async () => {
  try {
    const response = await await fetch(`${BASE_URLS.server}/get_weather`);
